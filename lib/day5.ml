...
let int_of_boarding_string str =
  let rec aux acc i =
...
let part1 (args :string list) :string =
  match args with
  | filename :: [] ->
    File.lines filename |> List.fold_left (fun acc str ->
        let seat_id = int_of_boarding_string str in
        if seat_id > acc then seat_id else acc
      ) 0 |>
    string_of_int
...
let part2 (args :string list) :string =
  ...
  | filename :: [] ->
    let seat_id_list =
      File.lines filename |>
      List.map (fun str -> int_of_boarding_string str) |>
      List.sort compare
    in
    if seat_id_list = [] then
      raise (Error.Bad_arg("Empty seat ID input list."));
    let rec find_seat last_seat = function
      | current_seat :: rest ->
        if (current_seat - last_seat) > 1 then
          last_seat + 1
        else
          find_seat current_seat rest
      | [] -> raise Not_found
    in
    find_seat (List.hd seat_id_list) seat_id_list |> string_of_int
  | _ -> raise (Error.Bad_arg("Puzzle requires data file argument."))
